...
def defaultDocumentObject = '''<?xml version="1.0" encoding="UTF-8"?>
<documentObject>
...
Given(~"an empty database"){
    browser.get("http://localhost:${System.getProperty('server.port', '9090') }/friark/auth/signIn?username=admin&password=admin")
    browser.get("http://localhost:${System.getProperty('server.port', '9090') }/friark/test/clearAll")
}
...
When(~"I POST a new object to the ([A-z]*) Controller"){ String ctrl ->
    switch(ctrl){
        ...
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName[ctrl]}.xml",defaultClassificationSystem)
        currentResult = browser.pageSource
        //println currentResult
        break
        case 'Klass':
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName['ClassificationSystem']}.xml",defaultClassificationSystem)
        ...
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName[ctrl]}.xml",klass)
        currentResult = browser.pageSource

        //println currentResult
        break
        case 'DocumentObject':
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName['DocumentDescription']}.xml",defaultDocumentDescription)
        def sys = new XmlSlurper().parseText(browser.pageSource)
        def docObj = new SimpleTemplateEngine().createTemplate(defaultDocumentObject).make([docId: sys.@id]).toString()
        browser.post("http://localhost:${System.getProperty('server.port','9090')}/friark/ws/${ctrlName[ctrl]}.xml",docObj)
        ...
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName[ctrl]}.xml",file)
        currentResult = browser.pageSource
        //println currentResult
        break
        ...
        def seriesRet = new XmlSlurper().parseText(browser.pageSource)
        def file = new SimpleTemplateEngine().createTemplate(defaultFile).make([parentId: seriesRet.@id]).toString()
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/file.xml",file)
        ...
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName[ctrl]}.xml",record)
        currentResult = browser.pageSource
        //println currentResult
        break
        case 'DocumentDescription':
        browser.post("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/documentDescription.xml",defaultDocumentDescription)
        currentResult = browser.pageSource
        //println currentResult
        break
        default:
...
Then(~"there should be (\\d+) ([A-z]*) in the database"){ int num, String domain ->
    browser.get("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/${ctrlName[domain]}.xml")
...
When(~"I search in ([A-z]+) for \"(.*)\" as OEP") { String domain, String query ->
    if(domain == "all") browser.get("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/search/${query}.oep")
    else browser.get("http://localhost:${System.getProperty('server.port', '9090') }/friark/ws/search/${domain}/${query}.oep")
    //println browser.pageSource
}
